...
static const float GB_CAM_MAX_FILLING_CLAMP = 1.50f; // 0 to 1 value for clamping the fill when camera is at max distance
static const float GB_THRESHOLD = 0.05f; // interpolation time
static const float GB_THRESHOLD2 = 0.25f; // interpolation time
//static const float3 GB_OUTLINE_COLOR = float3( 1.0f, 0.0f, 0.0f );
static const float GB_OUTLINE_CUTOFF_SEA = 1.0f; // Magic number to balance cutoff on edges without neighbor (over Sea)
...
static const float GB_STRENGTH_CH1 = 0.75; // Opacity of bottom layer
